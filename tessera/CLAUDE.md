...
The whole repo is Nix-managed. At the repo root, **`cp .envrc.example .envrc && direnv allow`** (`.envrc` is
per-developer and gitignored, #430; or plain `nix develop`) loads the
