...
| Bindings (pyo3 → C-ABI → WASM) | ◑ | Python read+verify+decode shipped (abi3 `import tessera`: open/manifest/blocks/read_block/**read_array**→numpy/**read_table**→numpy/verify + typed `TesseraError`); hermetic corpus import-check reconstructs every block via `numpy.frombuffer`. Write path + Arrow zero-copy + WASM pending | `tessera-py` (#210) |
