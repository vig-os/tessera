...
| Ingest: GE-HDF5 (listmode) | ✓ | lossless 2p+3p compound→columnar transpose (row-major #193 fix) + `ingested_from` provenance; CLI `tessera ingest ge-hdf5 --dataset`; **bounded-memory >RAM streaming** — `stream_events_2p` reads HDF5 row-slabs (hyperslab) → `TableStreamWriter` → seal, **byte-identical** to the whole-file read (the slab size never changes the canonical bytes). The reader is now a **generic HDF5-compound decoder** (#222) — any dataset (events/coin/singles/vendor) with zero per-record structs. **Real-DUPLET multi-GB scale measured** (`tessera bench write --input`): singles 294 M rows seals at 4.9 M events/s, input-bounded — but peak RSS = ~600 MiB + the compressed block (3.63 GiB @ singles) because the output block is materialised; **constant-memory >RAM** needs ADR-0026 sub-block output-streaming. x86==ARM determinism = wired CI matrix, awaits a green ARM run | **#208/#222**; `tessera_ingest::ge_hdf5::tests::{generic_read_compound_byte_identical_to_hardcoded_2p_and_3p,streamed_2p_ingest_is_byte_identical_to_whole_file_read}`; scale: `tessera/docs/SPIKE-RESULTS.md` |
